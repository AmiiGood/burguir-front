...
import { Star, Clock, Trophy, Tag, ShoppingCart, Check } from "lucide-react";
...
const ProductCard = ({ product }) => {
  ...
  const calculateFinalPrice = (price, discount) => {
    if (cart && cart.calculateFinalPrice) {
  ...
  return (
    ...
    >
      {showAddAnimation && (
        <motion.div
          className="absolute inset-0 bg-indigo-600/20 backdrop-blur-sm z-10 flex items-center justify-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
